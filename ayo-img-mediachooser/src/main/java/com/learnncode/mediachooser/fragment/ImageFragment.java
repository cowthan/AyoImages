...
public class ImageFragment extends Fragment
{
	...
	private void setAdapter()
	{
		...
		if (imageList != null && imageList.size() > 0) {
			...
			mImageAdapter = new GridViewAdapter(getActivity(), 0,  imageList, false,getArguments().getString("type"));
...
//				}
				if(galleryModel.status){
					...
					if(mSelectedItems.size() + DirectoryListActivity.imagesSelectedBefore.size() >= DirectoryListActivity.max){
						Toast.makeText(getActivity(), "最多只能选择" + DirectoryListActivity.max + "张图", Toast.LENGTH_SHORT).show();
						return;
					}
				}
				// inverse the status
				...
				CheckedTextView checkedTextView = (CheckedTextView) mImageGridView.findViewWithTag(galleryModel.url);
				if(checkedTextView != null){
					checkedTextView.setChecked(galleryModel.status);
				}
